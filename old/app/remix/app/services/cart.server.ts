...
const cartCookie = createCookie("__cart_session", {
    httpOnly: true,
    secure: true,
    secrets: [
...
export const useCart = {
    create: async (items: z.infer<typeof schema>): Promise<[ string, string ]> => {
        const parsedCart = schema.safeParse(items);

        if (parsedCart.error) return [ "Set-Cookie", await cartCookie.serialize(null, { maxAge: -1 }) ];

        return [ "Set-Cookie", await cartCookie.serialize([ ...parsedCart.data ]) ];
    },
    delete: async (request: Request): Promise<[ string, string ]> => {
        const parsedAuthToken = schema.safeParse(await cartCookie.parse(request.headers.get("Cookie")));

        return [ "Set-Cookie", await cartCookie.serialize("", { maxAge: -1 }) ];
    },
    parse: async (request: Request) => {
        const parsedCart = schema.safeParse(await cartCookie.parse(request.headers.get("Cookie")));

        if (parsedCart.error) return [];
