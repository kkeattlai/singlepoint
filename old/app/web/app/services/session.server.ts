...
export const session = {
    get: async (request: Request): Promise<z.infer<typeof validationSchema> | null> => {
        const user = validationSchema.safeParse(await sessionCookie.parse(request.headers.get("Cookie")));

        if (!user.success) return null;

        return user.data;
    },
    create: async (user: z.infer<typeof validationSchema>): Promise<[ string, string ]> => {
        return [ "Set-Cookie", await sessionCookie.serialize(user) ];
    }
};