...
const credentialCookie = createCookie("__rememberMe", {
    httpOnly: true,
    secure: true,
    secrets: [
...
export const useCredential = {
    create: async (credential: z.infer<typeof schema>): Promise<[ string, string ]> => {
        ...
        const { email, password, rememberMe } = parsedCredential.data;
        
        if (!rememberMe) return [ "Set-Cookie", await credentialCookie.serialize(null, { maxAge: -1 }) ];

        return [ "Set-Cookie", await credentialCookie.serialize({ email, password, rememberMe }) ];
    },
    parse: async (request: Request) => {
        const parsedCredential = schema.safeParse(await credentialCookie.parse(request.headers.get("Cookie")));

        if (parsedCredential.error) return null;
